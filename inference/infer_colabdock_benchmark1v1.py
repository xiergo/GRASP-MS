...
def get_colabdock_1v1_interface(pkl_path, seqlen, cutoff=8, fdr=0):
    with open(pkl_path, 'rb') as f:
        pos = pickle.load(f)
    if len(pos.shape)<2:
        pos = pos[None]
    ...
    n = int((bins<=cutoff).sum())
    x[:n] = (1-fdr)*x[:n]/(x[:n].sum())
    x[n:] = fdr*x[n:]/(x[n:].sum())
    x = x/x.sum()
...
if __name__ == "__main__":
    ...
    pdb_ids = [f'{i}_{j}' for i in os.listdir(restr_dir) if len(i)==4 for j in [1, 2, 3]]
