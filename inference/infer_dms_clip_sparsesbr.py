...
def get_dms_interface(restr_file, mapping, cdr_file=None, thre=0.2, cheat_mode=0, dimer=0, cdr=0, usesbr=0, noif=0):
    ...
    if cdr:
        ...
        if usesbr:
            ...
            pairs = [(i, j) for i in np.where(interface_mask==1)[0] for j in np.where(cdr_mask==1)[0]]
            sample_num = int(max(interface_mask.sum(), cdr_mask.sum()))
            # pairs = np.random.choice(pairs, sample_num, replace=False)
            pairs_sampled = [pairs[i] for i in np.random.choice(len(pairs), sample_num, replace=False)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jobname', default='dms_cliped_sparsesbr', type=str)
    ...
    # set path
    raw_feat_dir = f'{arguments.data_url}/dms/features'
    fasta_dir = f'{arguments.data_url}/dms/fasta'
    restr_dir = f'{arguments.data_url}/dms/dms'
    cdr_dir = f'{arguments.data_url}/dms/cdr'
    ...
    pdb_ids = [
        f'{pdb_id}_cheat{cheat}_thre{thre}_dimer0_cdr{cdr}_usesbr{usesbr}_noif{noif}' \
        for pdb_id in pdb_ids \
        for (cheat, thre, cdr, usesbr, noif) in [
            # (0, 0.0, 0, 0, 0), # RAW
            # (0, 0.2, 0, 0, 0), # QC
            # (1, 0.0, 0, 0, 0), # CHEAT
            # (0, 0.2, 1, 0, 0), # QC+CDR
            (0, 0.2, 1, 1, 0), # QC+CDR+SBR
            # (0, 0.2, 1, 1, 1), # QC+CDR+SBR+NOIF
        ]
        # for i in [0,] \
        # for j in [0.2,] \
        # for dimer in [0,] \
