...
def generate_split_first_num(split_file):
    with open(split_file, 'r') as f:
        split = [int(i) for i in f.read().strip().split('-')]
    print(split)
    return split[0]
...
class MyDataGenerator(DataGenerator):
    ...
    def get_pattern(self, pdb_id):
        ...
        restr_pat = f'{self.restr_dir}/{pdb_id}_rep{rep_id}.pkl'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jobname', default='bm5.5_extra', type=str)
    ...
    # set path
    raw_feat_dir = f'{arguments.data_url}/bm5.5_extra25/raw_feat/'
    fasta_dir = f'{arguments.data_url}/bm5.5_extra25/fasta/'
    restr_dir = f'{arguments.data_url}/bm5.5_extra25/restr'
    ...
    pdb_ids = [f'{i.split(".fasta")[0]}_{j}_dimer{dimer}' \
               for i in os.listdir(fasta_dir) if i.endswith('.fasta') \
               for j in [1, 2, 3] \
                for dimer in [0, 1]]
