...
class MyDataGenerator(DataGenerator):
    ...
    def get_pattern(self, pdb_id):
        ...
        pat_dict = {
            'raw_feat': f'{self.raw_feat_dir}/{pdb_id0}_dimer_{chain_pair}.pkl',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jobname', default='basic_benchmark_bl', type=str)
    ...
    # set path
    raw_feat_dir = f'{arguments.data_url}/basic_benchmark_dataset/raw_feature'
    ...
    restrs = glob.glob(f'{restr_dir}/*/*_raw.pkl')
    pdb_ids = [os.path.basename(i).replace('_raw.pkl', '') for i in restrs]
    pdb_ids = [i for i in pdb_ids if i.rsplit('_', 2)[0] in pdb_ids0]

    pdb_ids = [i for i in pdb_ids if '1v1' in i]
